...
int main(int argc, char* argv[])
{
	...
	TTF_Font* font = loadFont("assets\\Fonts\\arial.ttf", 1036584);
	if (font == NULL)
	{
		SDL_Log("Font not loaded");
	}

	SDL_Texture* text = renderText(renderer, "", font, color(255, 255, 255), FAST);
	...
	while (running) {
		...
		quickImage(renderer, text, 10, 10);
		...
		if (time(NULL) > frameStart)
		{
			...
			//text = renderText(renderer, buffer.c_str(), font, color(255, 255, 255), FAST);
